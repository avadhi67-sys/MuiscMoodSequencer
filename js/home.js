...
// Floating Glass Ribbon & Soft Aura Shimmer
// ============================================

function checkWelcomeCelebration() {
    const isJustAuth = sessionStorage.getItem("sonicflow_just_authenticated");
    if (!isJustAuth) return;

    // Immediately remove flag so it runs strictly once
    sessionStorage.removeItem("sonicflow_just_authenticated");

    let userName = "Music Explorer";
    try {
        const user = JSON.parse(localStorage.getItem("sonicflow_current_user") || "null");
        if (user && user.name) userName = user.name;
    } catch (e) {
        console.warn("Could not parse current user:", e);
    }

    // Trigger subtle hero ambient aura shimmer
    const heroEl = document.querySelector(".hero");
    if (heroEl) {
        heroEl.classList.add("hero-aura-active");
        setTimeout(() => heroEl.classList.remove("hero-aura-active"), 3500);
    }

    // Create Aesthetic Floating Glass Pill
    const pill = document.createElement("div");
    pill.className = "aesthetic-welcome-pill";
    pill.innerHTML = `
        <span class="welcome-pill-sparkle">✦</span>
        <div class="welcome-pill-content">
            <span class="welcome-pill-title">Welcome back, <strong>${escapeHTML(userName)}</strong></span>
            <span class="welcome-pill-sub"><span class="welcome-pulse-dot"></span> Studio Synchronized</span>
        </div>
        <span class="welcome-pill-wave">≋</span>
    `;

    document.body.appendChild(pill);

    // Spawn 20 fast, vibrant radiant stars & sparkles
    const sparklesWrapper = document.createElement("div");
    sparklesWrapper.className = "aesthetic-sparkles-wrapper";
    const symbols = ["✦", "★", "✨", "♪", "♫", "✸", "◆"];
    for (let i = 0; i < 20; i++) {
        const sparkle = document.createElement("span");
        sparkle.className = "aesthetic-sparkle-item";
        sparkle.textContent = symbols[i % symbols.length];
        sparkle.style.left = `${8 + Math.random() * 84}%`;
        sparkle.style.top = `${10 + Math.random() * 55}%`;
        sparkle.style.fontSize = `${16 + Math.random() * 18}px`;
        sparkle.style.animationDelay = `${(i * 0.035).toFixed(2)}s`;
        sparkle.style.animationDuration = `${0.85 + Math.random() * 0.45}s`;
        sparklesWrapper.appendChild(sparkle);
    }
    document.body.appendChild(sparklesWrapper);

    // Fast enter -> brief stay -> smooth disappear under 2s total
    setTimeout(() => pill.classList.add("show"), 60);

    setTimeout(() => {
        pill.classList.remove("show");
        pill.classList.add("hide");
        sparklesWrapper.classList.add("fade-out");
        setTimeout(() => {
            if (pill.parentNode) pill.parentNode.removeChild(pill);
            if (sparklesWrapper.parentNode) sparklesWrapper.parentNode.removeChild(sparklesWrapper);
        }, 300);
    }, 1450);
}
